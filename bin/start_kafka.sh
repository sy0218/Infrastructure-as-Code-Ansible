...
# ansible 플레이북 실행
if [ "${TARGET}" = "all" ]; then
    run_cmd "ansible-playbook kafka_ansible.yml"
else
    run_cmd "ansible-playbook kafka_ansible.yml --tags ${TARGET}"
fi

log "===== END kafka ansible end ====="
