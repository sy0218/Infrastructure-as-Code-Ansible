...
log "===== START terraform ansible start ====="
...
# ansible 플레이북 실행
if [ "${TARGET}" = "all" ]; then
    run_cmd "ansible-playbook terraform_ansible.yml"
else
    run_cmd "ansible-playbook terraform_ansible.yml --tags ${TARGET}"
fi

log "===== END terraform ansible end ====="
